...
function ViewController($scope, SharedService) {
        ...
        if (target.dataset.s3 === 'folder') {
            ...
            s3.getSignedUrl('getObject', params, (err, url) => {
                if (err) {
                    ...
                    var div = document.getElementById("fileview");
                    div.innerHTML = '<iframe style="width:100%;min-height:600px;border:none;overflow-y:hidden;overflow-x:hidden;margin:20px" frameborder="1" src="' + url + '" />';                }
            });
        }
        return false;
    });
    ...
    //<div class="box"><iframe src="https://en.wikipedia.org/" width = "500px" height = "500px"></iframe></div>
    $scope.renderObject = (data, _type, full) => {
        ...
        function buildAnchor(s3key, href, text, download) {
            ...
            a.text(text);
            DEBUG.log('print a tag', a.prop('outerHTML'));
            var popText ="";
            ///if(download) {
               // popText = '<div class="box"><iframe src="'  + href +  '"  width = "250px" height = "250px"></iframe></div>'; 
            //} 
            //return a.prop('outerHTML') + popText ;
    ...
    $scope.listobjectscb = (err, data) => {
        ...
        if (err) {
            ...
            $.each(data.CommonPrefixes, (index, value) => {
                DEBUG.log("CommonPrefixes:", data.CommonPrefixes);               
                DEBUG.log("Value:", value.Prefix);
                DEBUG.log("Index", index); 
                //DEBUG.log("Storage: ", StorageClass);
    ...
    this.renderSelect = (data, type, _full) => {
        DEBUG.log("renderSelect data", data.Key)
        if (type === 'display' && !data.Key.endsWith("/")  && !data.Key.includes("output")) {
            DEBUG.log("renderSelect  adding button", data.Key)
            return '<span class="button1"><input type="button" value="Process"></span>';
        }
    ...
    $('#s3objects-table tbody').on('click', 'input[type="button"]', (e1) => {
        //alert("pushing into queue");
        ...
        DEBUG.log('S3 Data information = ', data); 
        DEBUG.log("Wole Bucket =", $scope.settings);

        // Prevent click event from propagating to parent
        e1.stopPropagation();
/*
        $.post( "https://0c15daaf-default-apollopro-87d3-1684536731.us-east-1.elb.amazonaws.com/new_file", {"description": "file_upload", "name": "s3://apollo-source-data/local/ewan_mcgregor.wav"})
            .done(function( data ) {
...
/*
        ...
        xhr.open("POST", "0c15daaf-default-apollopro-87d3-1684536731.us-east-1.elb.amazonaws.com/new_file", false);
        ...
        xhr.send(JSON.stringify({"description": "file_upload", "name": "s3://apollo-source-data/local/ewan_mcgregor.wav"}
        ));
*/

        const file2process = "s3://" + $mybucket+"/" + data.Key;  
        DEBUG.log("SENDING Data with Jquery....bucket = ", file2process);

        var settings = {
            "url": "https://api.apollo-cttso.com/new_file/",
            "method": "POST",
            "timeout": 0,
            "headers": {
              "Content-Type": "application/json"
            },
            "data": JSON.stringify({"description":"virus_scan","name":file2process}),
          };
