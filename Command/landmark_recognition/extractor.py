...
def MakeExtractor(sess, config, import_scope=None):
  ...
  print(f'Load model in extractor.py: {load_finish - load_start}')
  ...
  def ExtractorFn(image):
    ...
    print(f'{len(image_scales)} image_scales: {image_scales}', flush=True)
    print(f'max feature num: {config.delf_local_config.max_feature_num}', flush=True)
    (locations_out, descriptors_out, feature_scales_out,
     attention_out) = sess.run(
         ...
         feed_dict={
             ...
             input_image_scales: image_scales,
