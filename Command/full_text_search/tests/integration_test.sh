...
services="flask-apollo-processor full-text-search"
...
# We can send an text file
curl -s -X POST localhost:8080/jobs/full_text_search/ \
     -H "authorization: Bearer $token" \
     -H 'Content-Type:application/json' -d '{"path": "inputs/ner/test.txt"}'

# Let the job finish
wait_until_database_ready "SearchFullText" 1

# search verification:
curl -s localhost:8080/search/full_text?query=crisis \
     -H "authorization: Bearer $token" \
     | grep Lebanon
