...
#export DOWNLOAD_MODEL_DURING_BUILD=1
services="flask-apollo-processor speech-to-text"
services="$services named-entity-recognition named-entity-recognition-search"
docker-compose build --build-arg DOWNLOAD_MODEL_DURING_BUILD --force-rm $services
...
# We can send an audio file:
curl -s -X POST localhost:8080/jobs/speech_to_text/ \
     -H "authorization: Bearer $token" \
     -H 'Content-Type:application/json' -d '{"path": "inputs/audio/demo3.wav"}'
curl -s -X POST localhost:8080/jobs/speech_to_text/ \
     -H "authorization: Bearer $token" \
     -H 'Content-Type:application/json' -d '{"path": "inputs/audio/bill_gates-TED.mp3"}'

# Let the job finish
wait_until_database_ready "SearchFullText" 2
wait_until_ner_ready 4

# search both epidemic OR (%7C = symbol '|') indian (should get demo3 and bill_gates)
curl -s localhost:8080/search/full_text?query=epidemic+%7C+indian \
     -H "authorization: Bearer $token" \
     | grep gates
curl -s localhost:8080/search/full_text?query=epidemic+%7C+indian \
     -H "authorization: Bearer $token" \
...
# check word timestamp
curl -s localhost:8080/search/full_text?query=epidemic \
     -H "authorization: Bearer $token" \
     | grep -1 timestamp | grep 56.9

# search "indian" in NER:
curl -s http://localhost:8080/search/ner?entity=indian \
     -H "authorization: Bearer $token" \
     | grep demo3
