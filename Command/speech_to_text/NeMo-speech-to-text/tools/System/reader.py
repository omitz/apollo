...
class Reader(CommonReader):
  ...
  def new(project_id):
    ...
    # Acoustic model
    manifest.am = edict()
    manifest.am.train_dataset_name = None
    manifest.am.train_dataset = None
  ...
  def get_am_train_cmd(self):
    ...
    """
    config_changes = []
    if self.manifest.am.train_dataset == None:
    ...
    # datasets
    config_changes.append('--train_dataset=' + str(self.manifest.am.train_dataset))
    if len(self.manifest.eval_datasets) != 0:
      config_changes.append('--eval_datasets=' + str(','.join(self.manifest.eval_datasets)))
  ...
  def add_dataset(self, dataset_file, dataset_id, dataset_type="am-train"):
    ...
    if dataset_type == "am-train":
      if dataset_id == self.manifest.am.train_dataset_name:
        print (dataset_id, 'is already included in the am-train dataset - replacing it')

      self.manifest.am.train_dataset = dataset_file
      self.manifest.am.train_dataset_name = dataset_id
      ...
      if dataset_id == self.manifest.lm.train_dataset_name:
        print (dataset_id, 'is already included in the lm-train dataset - replacing it')
