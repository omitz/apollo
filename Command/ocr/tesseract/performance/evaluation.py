...
def ConductExp (dataSetDir):
    ...
    failures = {}
    totalClassCount = {}
    # tmpOutFile = "/dev/shm/tesseractResult.txt"
    ...
    for lang in langCategories:
        ...
        for imgFile in dataset [lang]:
            ...
            print ("tesseract_main.run...")
            tmpOutFile = "/dev/shm/" + os.path.basename (imgFile)
            # succeed = tesseract_main.run (Path (imgFile), Path (tmpOutFile), "delme.png")
            print (f"set language to {to_tess_lang[lang]}")
            succeed = tesseract_main.run (Path (imgFile), Path (tmpOutFile),
                                          "delme.png", None, to_tess_lang[lang])
            assert (succeed)
...
if __name__ == "__main__":
    ...
    # (failures, accuracies) = ConductExp ("../tests/testDataset.easy-ocr")
