...
def process_file(s3_file_path: str):
    ...
    debug_Path = Path(Path(target).name.replace('.', '_') + "_ocr-tesseract-overlay.png")
    ...
    succeed = tesseract_main.run (ram_target_Path, ram_output_Path, ram_debug_Path)
...
if __name__ == '__main__':
    ...
    if args.debug:
        tesseract_main.run ("tests/images/ocr.gif", "out.txt", "out.png")
        sys.exit (0)
    
    bunny = RabbitWorker('ApolloExchange', SERVICE_NAME)
    bunny.service_fullname = 'Tesseract OCR'
