...
class OcrTest(unittest.TestCase):
    ...
    def test_decode(self):
        ...
        if os.getenv ("JENKINS"): # inside Docker container already
            ...
            succeed = tesseract_main.run (imagePath, outTxtPath, outImgPath)
            self.assertTrue(succeed == True)
            self.assertTrue("brown fox" in outTxtPath.read_text())
        else:
            retExec = self.container.exec_run (
                "./tesseract_main.py tests/images/ocr.png " +
                "/dev/stdout out.png debug.txt")
    ...
    def test_ocr (self):
        print ("Testing more decode.")
        if os.getenv ("JENKINS"): # inside Docker container already
            retVal = os.system ("python -m unittest tests.ocrTest")
            self.assertTrue(retVal == 0)
        else:
            retExec = self.container.exec_run (
                "python -m unittest tests.ocrTest")
            self.assertTrue(retExec.exit_code == 0)
