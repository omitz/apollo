...
class EasyOcrAnalytic (Analytic):
    s3filestore = None
    ram_storage = '/dev/shm'
    ...
    def run (self, s3_file_path: str):
        ...
        output_Path = Path(Path(target).name.replace('.', '_') + "_ocr-easy.txt")
        ...
        print ("easyOcr_main.run...", flush=True)
        succeed = easyOcr_main.run (ram_target_Path, ram_output_Path, None, ram_metadata_Path)

        if not succeed:
            print ("No text detected OR Detection failed!", flush=True)
