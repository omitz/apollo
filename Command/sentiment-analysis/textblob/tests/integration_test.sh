...
services="flask-apollo-processor sentiment-textblob"
...
wait_until_rabbitmq_connections 1
...
# We can send text file:
curl -s -X POST localhost:8080/jobs/text_sentiment/ \
     -H "authorization: Bearer $token" \
     -H 'Content-Type:application/json' -d '{"path": "inputs/sentiment-analysis/negative_v2.txt"}'
curl -s -X POST localhost:8080/jobs/text_sentiment/ \
     -H "authorization: Bearer $token" \
     -H 'Content-Type:application/json' -d '{"path": "inputs/sentiment-analysis/positive.txt"}'

# Let the job finish
wait_until_database_ready "AnalyzeSentiment" 2

# search the result
curl -s localhost:8080/search/text_sentiment?sentiment=positive \
     -H "authorization: Bearer $token" \
     | grep positive.txt

curl -s localhost:8080/search/text_sentiment?sentiment=negative \
     -H "authorization: Bearer $token" \
     | grep negative_v2.txt
