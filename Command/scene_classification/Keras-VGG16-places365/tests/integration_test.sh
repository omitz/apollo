...
services="flask-apollo-processor scene-places365"
...
curl -s -X POST localhost:8080/jobs/scene_classification/ \
     -H "authorization: Bearer $token" \
     -H 'Content-Type:application/json'  -d '{"path": "inputs/scene-classification/6.jpg"}'

# Let the job finish
# echo "add delay for processing to complete.."
# echo "(warning: sleep is not a robust way to ensure readiness)"
# add_delay 10
wait_until_database_ready "ClassifyScene" 1

# test search 
curl -s localhost:8080/search/scene_hierarchy?tag=indoor \
     -H "authorization: Bearer $token" \
     | grep "6.jpg"
curl -s localhost:8080/search/scene_class?tag=food+court \
     -H "authorization: Bearer $token" \
     | grep "6.jpg"
