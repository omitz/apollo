...
ENROLL_LIST_FILE = "enroll_list.csv"
TEST_LIST_FILE = "test_list.csv"
COST_METRIC = "cosine"
...
def LoadEntrollmentData (savedEnrollModel="enrollmentModel_Vosk.pkl"):
    ...
    if os.path.isfile (savedEnrollModel):
        ...
        enroll_result = get_embeddings_from_list_file (ENROLL_LIST_FILE)
...
def NearestNeighborClassifier (enroll_embs, speakers, test_embs, testListFile):
    ...
    distances_mat = pd.DataFrame (
        cdist (test_embs, enroll_embs, # 10 rows (test) by 200 columns (enroll) 
               metric=COST_METRIC), columns=speakers) # name of each column
...
if __name__ == '__main__':
    ...
    embedding from the test dataset, specified by "TEST_LIST_FILE".
    """
    ...
    if args.resultFile == None:
        ...
        (dirName, fileName) = os.path.split(RESULT_FILE)
