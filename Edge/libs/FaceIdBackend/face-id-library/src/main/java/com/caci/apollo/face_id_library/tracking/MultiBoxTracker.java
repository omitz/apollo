...
package com.caci.apollo.face_id_library.tracking;
...
import com.caci.apollo.face_id_library.Classifier.Recognition;
import com.caci.apollo.face_id_library.env.BorderedText;
import com.caci.apollo.face_id_library.env.ImageUtils;
import com.caci.apollo.face_id_library.env.Logger;
import com.caci.apollo.face_id_library.utils.ColorThresholds;

/**
 * A tracker wrapping ObjectTracker that also handles non-max
 * suppression and matching existing objects to new detections.
 */
public class MultiBoxTracker {
//    private final Logger logger = new Logger();
    private static String TAG = "Tommy MultiBoxTracker";
    
    private static final float TEXT_SIZE_DIP = 18;

    // Maximum percentage of a box that can be overlapped by another
    // box at detection time. Otherwise the lower scored box (new or
    // old) will be removed.
    ...
    public synchronized void drawDebug(final Canvas canvas) {
        ...
        for (final TrackedRecognition recognition : trackedObjects) {
            ...
            if (getFrameToCanvasMatrix().mapRect(trackedPos)) {
                final String labelString = String.format("%.2f",
    ...
    public synchronized void trackResults (final List<Recognition> results,
        ...
        // Log.d (TAG, String.format ("Processing %d results from %d", results.size(), timestamp));
        processResults (timestamp, results, frame);
    }

    public synchronized void draw (final Canvas canvas) {
        final boolean rotated = sensorOrientation % 180 == 90; // 90, 270, etc. exactly? TBF
        ...
        frameToCanvasMatrix =
                ImageUtils.getTransformationMatrix(
                        ...
                        sensorOrientation,
                        false);
        // Log.d (TAG, "frameWidth = " + frameWidth + " frameHeight = " + frameHeight);
        // Log.d (TAG, "draw rotated = " + rotated + " multiplier = " + multiplier);
        // Log.d (TAG, "frameToCanvasMatrix = " + frameToCanvasMatrix);
        // Log.d (TAG, "len trackedObjects = " + trackedObjects.size());
        ...
        for (final TrackedRecognition recognition : trackedObjects) {
            ...
            // Log.d (TAG, "trackedPos = " + trackedPos); // TC 2021-04-05 (Mon) -- should change
            ...
            borderedText.drawText(canvas, trackedPos.left + cornerSize,
                                  trackedPos.bottom, labelString);
            // Log.d (TAG, "draw with labelString = " + labelString);
        }
    }
    ...
    public synchronized void onFrame(
        ...
        if (objectTracker == null && !initialized) {
            ...
            // Log.d (TAG, String.format ("Initializing ObjectTracker: %dx%d", w, h));
        ...
        // Log.d (TAG, "calling objectTracker.nextFrame = " + timestamp);
        ...
        for (final TrackedRecognition recognition : copyList) {
            ...
            // Log.d (TAG, "correlation = " + correlation);
            if (correlation < MIN_CORRELATION) {
                // logger.v("Removing tracked object %s because NCC is %.2f",
                //          trackedObject, correlation);
                Log.d(TAG, String.format ("Removing tracked object %s because NCC is %.2f",
                                          trackedObject, correlation));
    ...
    private void processResults(final long timestamp,
        ...
        for (final Recognition result : results) {
            ...
            // logger.v("Result! Frame: " + result.getLocation() +
        ...
        if (objectTracker == null) {
            ...
            for (final Pair<Float, Recognition> potential : rectsToTrack) {
                ...
                trackedRecognition.color = ColorThresholds.setColorBasedOnConfidence
                    (trackedRecognition.detectionConfidence); // // APOLLO MOD
        ...
        // Log.d (TAG, "rects to track = " + rectsToTrack.size());
        for (final Pair<Float, Recognition> potential : rectsToTrack) {
            handleDetection (originalFrame, timestamp, potential);
        }
    }

    private void handleDetection (final byte[] frameCopy, final long timestamp,
        ...
        // find the max correlation in the image
        ...
        //         potential.second, potentialObject.getTrackedPositionInPreviewFrame(),
        ...
        if (potentialCorrelation < MARGINAL_CORRELATION) {
            ...
            Log.d(TAG, "Correlation too low to begin tracking " + potentialObject);
        ...
        // This is the current tracked object whose color we will
        // take. If left null we'll take the first one from the color
        // queue.
        TrackedRecognition recogToReplace = null;

        // Look for intersections that will be overridden by this
        // object or an intersection that would prevent this one from
        // being placed.
        for (final TrackedRecognition trackedRecognition : trackedObjects) {
            ...
            // If there is an intersection with this currently tracked
            // box above the maximum overlap percentage allowed,
            // either the new recognition needs to be dismissed or the
            // old recognition needs to be removed and possibly
            ...
            if (intersects && intersectOverUnion > MAX_OVERLAP) {
                if (potential.first < trackedRecognition.detectionConfidence
                    ...
                    // If track for the existing object is still going
                    // strong and the detection score was good, reject
                    // this new object.
                    potentialObject.stopTracking();
                    return;
                } else {
                    ...
                    // Let the previously tracked object with max
                    // intersection amount donate its color to the new
                    // object.
        ...
        // If we're already tracking the max object and no
        // intersections were found to bump off, pick the worst
        // current tracked object to remove, if it's also worse than
        ...
        if (availableColors.isEmpty() && removeList.isEmpty()) {
            ...
            if (recogToReplace != null) {
                ...
                Log.d (TAG, "Found non-intersecting object to remove.");
                removeList.add(recogToReplace);
            } else {
                // logger.v("No non-intersecting object found to remove");
                Log.d (TAG, "No non-intersecting object found to remove");
            }
        }
        ...
        if (recogToReplace == null && availableColors.isEmpty()) {
            ...
            Log.d (TAG, "No room to track this object, aborting.");
            potentialObject.stopTracking();
            return;
        }
        ...
        trackedRecognition.color = ColorThresholds.setColorBasedOnConfidence(trackedRecognition.detectionConfidence); // APOLLO MOD
        trackedObjects.add(trackedRecognition);
    }
}
