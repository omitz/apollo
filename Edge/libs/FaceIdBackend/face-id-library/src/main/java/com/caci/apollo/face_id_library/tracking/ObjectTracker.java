...
import com.caci.apollo.face_id_library.env.Size;
...
public class ObjectTracker {
    ...
    private static String TAG = "Tommy ObjectTracker";
    ...
    static {
        try {
            ...
            Log.d (TAG, "libtensorflow_demo.so not found, tracking unavailable");
        }
    }
    ...
    public static synchronized ObjectTracker getInstance(
        ...
        if (!libraryFound) {
            ...
            Log.d (TAG, "Native object tracking support not found. "
                   + "See tensorflow/examples/android/README.md for details.");
            return null;
        }
    ...
    public synchronized void nextFrame(
        ...
        if (downsampledTimestamp != timestamp) {
            ObjectTracker.downsampleImageNative(
                    frameWidth, frameHeight, rowStride, frameData,
                    DOWNSAMPLE_FACTOR, downsampledFrame);
        ...
        timestampedDeltas.add(new TimestampedDeltas(timestamp,
                                                    getKeypointsPacked(DOWNSAMPLE_FACTOR)));
        ...
        synchronized void setPreviousPosition(final RectF position, final long timestamp) {
            ...
            synchronized (ObjectTracker.this) {
                if (lastExternalPositionTime > timestamp) {
                    ...
                    Log.d (TAG, "Tried to use older position time!");
                    return;
                }
                final RectF externalPosition = downscaleRect(position);
                lastExternalPositionTime = timestamp;
