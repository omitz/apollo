...
public class HelloWorldDropDownReceiver extends DropDownReceiver implements
    ...
    public static final String TAG = "Tommy HllWrldDrpDwnRcvr";
    ...
    public HelloWorldDropDownReceiver(final MapView mapView,
        ...
        final Button speakerID = helloView.findViewById(R.id.speakerID);
        speakerID.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (DataFeedAPI.HasActivity ("SpeakerIdActivity")) {
                    ...
                    // TC 2021-04-10 (Sat) -- add speakerID here
                    Intent intent = new Intent();
                    intent.setClassName   ("com.atakmap.android.helloworld.plugin",
                                           "com.atakmap.android.helloworld.SpeakerIdActivity");
    ...
    @Override
    public void onReceive(Context context, Intent intent) {
        // Called when a broadcast signal is received
        ...
        if (inputs != null) {
            for (Bundle input : inputs)
                Log.d(TAG, "input " + input.getString(CotPort.DESCRIPTION_KEY)
                        ...
                        + ": " + input.getBoolean(CotPort.ENABLED_KEY)
                        + ": " + input.getBoolean(CotPort.CONNECTED_KEY));
        }
        if (outputs != null) {
            for (Bundle output : outputs)
                Log.d(TAG, "output " + output.getString(CotPort.DESCRIPTION_KEY)
                        ...
                        + ": " + output.getBoolean(CotPort.ENABLED_KEY)
                        + ": " + output.getBoolean(CotPort.CONNECTED_KEY));
        }
        if (streams != null) {
