...
public class SpeakerIdActivity extends Activity {
    ...
    private void init () {
        ...
        // 2.) Load the models
        try {
            // m_spkrID.LoadModels(ctx); // from assets
            m_spkrID.LoadModels(ctx, "speakerID_celebrity10"); // from assets
        } catch (IOException e) {
            e.printStackTrace();
            Log.d(TAG, "Error calling LoadModels");
            Toast.makeText(ctx, "Faild to load speakerID_celebrity10 model. " +
                           "  Please make sure the model data package is installed.",
