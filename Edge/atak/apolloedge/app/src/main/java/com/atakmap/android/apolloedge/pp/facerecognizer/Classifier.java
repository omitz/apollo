...
import com.atakmap.android.apolloedge.pp.facerecognizer.wrapper.FaceNet;
import com.atakmap.android.apolloedge.pp.facerecognizer.wrapper.LibSVM;
import com.atakmap.android.apolloedge.pp.facerecognizer.wrapper.MTCNN;
...
public class Classifier {
    ...
    static Classifier getInstance (AssetManager assetManager,
        ...
        /*
        assetManager has the data file (presumably the embeddings for Angela Merkel, etc), the model file, and the label file (a list of names)
         */
        if (classifier != null) return classifier;

        LOGGER.i("!!!!!!Classifier -> Before creating new Classifier");

        classifier = new Classifier();
        LOGGER.i("!!!!!!Classifier -> after creating new Classifier - b/4 MTCNN.create(assetManager)");

        classifier.mtcnn = MTCNN.create(assetManager);

        LOGGER.i("!!!!!!Classifier -> after MTCNN.create(assetManager)");

        classifier.faceNet = FaceNet.create(assetManager, inputHeight, inputWidth);
        classifier.svm = LibSVM.getInstance();
        LOGGER.i("!!!!!!Classifier -> Before reading label");
    ...
    List<Recognition> recognizeImage(Bitmap bitmap, Matrix matrix) {
        synchronized (this) {
            ...
            for (Pair face : faces) {
                ...
                String name;
                if (prob > ColorThresholds.LOW_CONFIDENCE_THRESHOLD && classNames.size() > 0)
                    name = classNames.get(pair.first);
                else
//                    name = "Unknown.\nClosest match:\n" + classNames.get(pair.first);
                    name = "Unknown";
    ...
    void updateData(int label, ContentResolver contentResolver, ArrayList<Uri> uris) throws Exception {
        synchronized (this) {
            ...
            for (Uri uri : uris) {
                Bitmap bitmap = FileUtils.getBitmapFromUri(contentResolver, uri);
                Pair faces[] = mtcnn.detect(bitmap);
